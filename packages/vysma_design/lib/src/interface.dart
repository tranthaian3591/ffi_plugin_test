...
/// The exposed API to interact with VysmaDesign
...
final VysmaDesignLib = VysmaDesignInterface._();

/// The interface of the API to interact with VysmaDesign
class VysmaDesignInterface {
  ...
  Future<VysmaDesignInstance> getInstance({
    ...
    _vysmaDesign ??= createWrapperImpl(library);
    // await _milli!
    //     .ensureInstanceInitialized(instanceDir: path, tmpDir: tmpDir());
    return _instances.putIfAbsent(
      path,
      () => VysmaDesignInstanceImpl(path, _vysmaDesign!),
    );
  }
}
